...
// Configura la conexion Sequelize (base de datos SQLite en memoria)
...
app.get("/api/museos", async (req, res) => {
  try {
    ...
    res.status(500).send({ message: "Error al recuperar los museos" });
  }
});
...
// Endpoint para agregar un museo
app.post("/api/museos", async (req, res) => {
  try {
    const museo = await Museo.create(req.body);
    res.json(museo);
  } catch (error) {
    res.status(500).send({ message: "Error al agregar el museo" });
  }
});

// Endpoint para eliminar un museo
app.delete("/api/museos/:id", async (req, res) => {
  try {
    const id = req.params.id;
    await Museo.destroy({ where: { id } });
    res.sendStatus(204); // Sin contenido
  } catch (error) {
    res.status(500).send({ message: "Error al eliminar el museo" });
  }
});
