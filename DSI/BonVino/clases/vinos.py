...
class Vino:
    ...
    def crear_varietal(varietales):
        for varietal in varietales:
            Varietal(
                varietal.descripcion, varietal.porcentaje_composicion, varietal.tipo_uva
            )

    def __str__(self):
        varietales_str = "\n".join(
            [
                f"  - {varietal.descripcion} ({varietal.porcentaje_composicion}%) - Tipo de uva: {varietal.tipo_uva.nombre}"
        ...
        maridajes_str = "\n".join(
            [
                f"  - {maridaje.nombre}: {maridaje.descripcion}"
                for maridaje in self.maridaje
            ]
        )
